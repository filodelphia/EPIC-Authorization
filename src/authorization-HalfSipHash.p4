...
const bit<8> HOP_AUTH = 254;

#ifndef MAX_SRV6_SEGMENTS
    #define MAX_SRV6_SEGMENTS 10
#endif
#if MAX_SRV6_SEGMENTS > 16
	#error "MAX_SRV6_SEGMENTS too large for macros"
#endif

const bit<32> CLOCK_SKEW = 32w300; // 5 minutes in seconds
...
header route_segment_list_entry_h {
    bit<128> address;
}
...
header epic_hop_authenticator_h {
	bit<32> calculated_hop_authenticator;
	bit<8> nextHeader;
}
...
struct headers_t {
    ...
    // Route headers
    route_base_h route_header;
    route_segment_list_entry_h[MAX_SRV6_SEGMENTS] segment_list;

    // EPIC headers
    epic_h epic;
    epic_hop_authenticator_h epic_hop_auth;
	epic_per_hop_h epic_per_hop;


	mac_check_h mac_check;
}
...
parser IngressParser(
    ...
    state start {
		ig_md.ext_idx = 0;
		ig_md.segment_list_count = 0;
		ig_md.sr_index = 0;
		ig_md.rnd_port_for_recirc = 0;
		ig_md.rnd_bit = 0;
		ig_md.rotated_k1 = 0;

		ig_md.sip_tmp.a_0 = 0; ig_md.sip_tmp.a_1 = 0; ig_md.sip_tmp.a_2 = 0; ig_md.sip_tmp.a_3 = 0;
		ig_md.sip_tmp.i_0 = 0; ig_md.sip_tmp.i_1 = 0; ig_md.sip_tmp.i_2 = 0; ig_md.sip_tmp.i_3 = 0;
    ...
    state parse_ethernet {
		...
		transition select(hdr.ethernet.etherType){
			...
			TYPE_SIP:  parse_sip_maccheck;
			default: accept;
		}
    }

	state parse_sip_maccheck {
		packet.extract(hdr.sip);
		packet.extract(hdr.sip_meta);
		packet.extract(hdr.mac_check);
		transition accept;
	}
	...
	//#define transitions transition parse_segment_##i
    state parse_route {
        packet.extract(hdr.route_header);
		ig_md.ext_idx = 0;
		transition parse_epic;
    }
	...
	state parse_segment {
		...
		transition select(ig_md.ext_idx < (hdr.route_header.last_entry + 1)) {
			true: parse_segment;
			false: parse_epic;
		}
	}*/
    ...
    state parse_epic {
        ...
        transition select(hdr.epic.per_hop_count){
            ...
            default: epic_recirculation_choice;
        }
    }

	state epic_recirculation_choice {
		transition select(hdr.epic.nextHeader){
			HOP_AUTH: parse_epic_hop_auth;
			default: parse_epic_hop;
		}
	}

	state parse_epic_hop_auth {
		packet.extract(hdr.epic_hop_auth);
		transition parse_epic_hop;
	}
...
parser EgressParser(packet_in packet,
    ...
    state start {
		eg_md.sip_tmp.a_0 = 0; eg_md.sip_tmp.a_1 = 0; eg_md.sip_tmp.a_2 = 0; eg_md.sip_tmp.a_3 = 0;
		eg_md.sip_tmp.i_0 = 0; eg_md.sip_tmp.i_1 = 0; eg_md.sip_tmp.i_2 = 0; eg_md.sip_tmp.i_3 = 0;
		eg_md.sip_tmp.round_type = 0;
	...
	state parse_sip {
		packet.extract(hdr.sip);
		packet.extract(hdr.sip_meta);
		transition parse_mac_check;
	}

	state parse_mac_check {
		packet.extract(hdr.mac_check);
		transition accept;
	}
}
...
control Ingress(inout headers_t hdr,
	...
	/* Unallowed by compiler, unrolling...
     * hdr.ipv6.dstAddr = hdr.segment_list[index].address;
	 */
	
	/*
	#define GEN_NEXT_DEST(i) \
	action nextDestination_##i() { \
		hdr.ipv6.dstAddr = hdr.segment_list[i].address; \
		hdr.route_header.segmentsLeft = hdr.route_header.segmentsLeft - 1; \
	}
	__LOOP(MAX_SRV6_SEGMENTS, GEN_NEXT_DEST)
	*/
    ...
    // Routing table
	#define GEN_DEST_TABLE(i) nextDestination_##i;

	/*
    table sr_forwarding {
        key = {
            hdr.ipv6.dstAddr: exact;
			ig_md.sr_index: exact;
        }

        actions = {
			__LOOP(MAX_SRV6_SEGMENTS, GEN_DEST_TABLE)
            nop;
        }
	...
	*/
	...
	action sip_hop_authenticator(){
		...
		hdr.sip.m_1 = hdr.epic_per_hop.tsexp
					++ hdr.epic_per_hop.ingress_if
					++ hdr.epic_per_hop.egress_if
					++ (bit<8>) hdr.epic_per_hop.segment_identifier[15:8];
		
		hdr.sip.m_2 = (bit<8>) hdr.epic_per_hop.segment_identifier[7:0]
					++ (bit<24>) 0;
		hdr.sip.m_3 = 0;
	}

	action sip_packet_authorization(){
		hdr.sip.setValid();
		...
		hdr.mac_check.setValid();
		hdr.mac_check.epic_mac = hdr.epic_per_hop.hop_validation;
	}
	...
	action start_first_pass(bit<32> key1, bit<32> key2){
		// first pass init
    ...
    apply {
		bool do_ipv6_forwarding = false;
		bool do_sr_forwarding   = false;
		bool do_halfsiphash     = false;
		...
		if(hdr.epic.isValid() && !hdr.sip.isValid() && !hdr.sip_meta.isValid()) {
			// Start Halfsip hash, either for the hop authenticator or the per-packet authorization
			if(!(ig_intr_md.ingress_port == 68 || ig_intr_md.ingress_port == 68 + 128)) {
				// Came in from outside, start calulcating hop authenticator
				sip_hop_authenticator();
				start_first_pass(KEY_0, KEY_1);

				do_halfsiphash = true;
			} else if(hdr.epic_hop_auth.isValid()) {
				// Calculated hop authenticator, start calculating per-hop authorization
				// Came in from recirculation port, calculating per-packet authorization hop authenticator
				ig_md.rotated_k1 = hdr.epic_hop_auth.calculated_hop_authenticator[15:0]
								 ++ hdr.epic_hop_auth.calculated_hop_authenticator[31:16];
				
				sip_packet_authorization();
				start_first_pass(hdr.epic_hop_auth.calculated_hop_authenticator, ig_md.rotated_k1);

				do_halfsiphash = true;
			} else {
				// MAC is correct, apply SRH and forward packet
				if(hdr.ipv6.isValid()) {
					do_ipv6_forwarding = true;
					
					/*
					if(hdr.route_header.isValid() && hdr.route_header.segmentsLeft > 0) {
						ig_md.sr_index = ig_md.segment_list_count - hdr.route_header.segmentsLeft;
						do_sr_forwarding = true;
					}
					*/
				}

				/* TODO: Check Time validity, removing for now
				// Time Validation
				bit<48> time_diff;
				bit<32> raw_shift;
				bit<32> adjusted_time;
				bool ts_is_valid = false;

				// Calculating time in seconds from ingress_global_timestamp (in nanoseconds)
				raw_shift = (bit<32>)(ig_intr_md.ingress_global_timestamp >> 30);
				adjusted_time = raw_shift + (raw_shift >> 4) + (raw_shift >> 7);


				if (hdr.epic.packet_ts >= adjusted_time)
					time_diff = hdr.epic.packet_ts - adjusted_time;
				else time_diff = adjusted_time - hdr.epic.packet_ts;

				if (time_diff > CLOCK_SKEW) {
				    drop();
				    exit;
				}
		...
		// Packet is neither SIP nor EPIC -> Simple IPv6 + SRH forwarding
		if (!hdr.epic.isValid()) {
			do_ipv6_forwarding = true;
			if (hdr.route_header.isValid() && hdr.route_header.segmentsLeft > 0) do_sr_forwarding = true;
		}

		if(do_ipv6_forwarding){
		...
		*/
		...
		// 3. Now we know: EPIC + SIP present sip_meta MUST be valid if we created SIP correctly.
		...
		// Apply recirc desicion and override in special case to process EPIC
...
control Egress(inout headers_t hdr,
	...
	apply {
		...
		if(hdr.sip_meta.curr_round < (NUM_WORDS*2+2)){
			...
			if (mac24 != hdr.mac_check.epic_mac) {
...
control IngressDeparser(
	...
	apply {
		...
		// Pass mac information from Ingress to Egress
		packet.emit(hdr.mac_check);
		...
		/*
		 * The following headers are emitted only during recirculations.
		 * Once the MAC has been checked and the packet will be forwarded, they will be removed to optimize space
		*/

		packet.emit(hdr.epic_hop_auth);
