...
def run_tests():
    ...
    try:
        send_and_expect(pkt2, marker, True, good_sniff, "VALID_4_EPIC", "02_valid")
    ...
    marker = b"T_BAD_HVF_" + struct.pack("!I", (int(time.time()) + 1) & 0xFFFFFFFF)

    hvf24 = epic.get_pkt_mac()
    epic_pkt = epic.build_epic(hvf_override=(hvf24 ^ 0x1))
    srh_pkt = srh.build_srh()
    pkt3 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
    ...
    try:
        send_and_expect(pkt3, marker, False, drop_sniff, "BAD_HVF", "03_bad_hvf")
    ...
    marker = b"T_BAD_SRC_" + struct.pack("!I", (int(time.time()) + 2) & 0xFFFFFFFF)
    epic_pkt = epic.build_epic(src_override=(SRC_AS_HOST ^ 0x1))
    srh_pkt = srh.build_srh()
    pkt4 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
    ...
    try:
        send_and_expect(pkt4, marker, False, drop_sniff, "BAD_SRC", "04_bad_src")
    ...
    marker = b"T_BAD_SEGID_" + struct.pack("!I", (int(time.time()) + 3) & 0xFFFFFFFF)

    epic_pkt = epic.build_epic(segid_override=(SEG_ID ^ 0x1))
    srh_pkt = srh.build_srh()
    pkt5 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
    ...
    try:
        send_and_expect(pkt5, marker, False, drop_sniff, "BAD_SEGID", "05_bad_segid")
    ...
    # 6) BAD_SEGLEFT
    marker = b"T_BAD_SEGLEFT_" + struct.pack("!I", (int(time.time()) + 4) & 0xFFFFFFFF)
    ...
    try:
        send_and_expect(pkt6, marker, False, drop_sniff, "BAD_SEGLEFT", "06_bad_segleft")
    ...
    marker = b"T_BAD_INGPORT_" + struct.pack("!I", (int(time.time()) + 5) & 0xFFFFFFFF)
    epic_pkt = epic.build_epic(ingress_port_override=1)
    srh_pkt = srh.build_srh()
    pkt7 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
    ...
    try:
        send_and_expect(pkt7, marker, False, drop_sniff, "BAD_ING_PORT", "07_bad_ing_port")
    ...
    marker = b"T_BAD_EGPORT_" + struct.pack("!I", (int(time.time()) + 5) & 0xFFFFFFFF)
    epic_pkt = epic.build_epic(egress_port_override=4)
    srh_pkt = srh.build_srh()
    pkt8 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
    ...
    try:
        send_and_expect(pkt8, marker, False, drop_sniff, "BAD_EG_PORT", "08_bad_eg_port")
    ...
    epic.pkt_ts = PKT_TS + 0x400; epic_pkt = epic.build_epic()
    ...
    epic.pkt_ts = PKT_TS + 0x500; epic_pkt = epic.build_epic(hvf_override=pkt9_1_hvf)
    pkt9_2 = (
        Ether(src=SRC_MAC, dst=DST_MAC, type=0x86DD) /
        ipv6 /
        srh_pkt /
